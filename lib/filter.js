...
function valid(obj, query) {
    ...
    for (let i = 0, l = tests.length; i < l; i += 2) {
        ...
        // perform filter test
        if (truthy) {
            // also test stringified value
            value = value === obj[key] || value === "" + obj[key];
        } else {
            // exception undefined is not matched for negated non-undefined values
            value = (value !== obj[key] && (obj[key] !== undefined || key === undefined));
        }
