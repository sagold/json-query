...
function set(data, queryString, value) {
    ...
    path.forEach((query, index) => {
        if (isProperty.test(query)) {
            ...
            // console.log(query, "->", nextKey, "array?", isArray.test(nextKey) || isIndex.test(nextKey));

            if (isArray.test(nextKey) || isIndex.test(nextKey)) {
                workingSet = create(workingSet, query, true);
            } else {
